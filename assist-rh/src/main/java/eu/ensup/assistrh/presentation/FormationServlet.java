...
import eu.ensup.assistrh.domaine.Formation;
	...
	public class FormationServlet extends HttpServlet {
		...
		protected void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			System.out.println("servlet formation");
			String id = request.getParameter("id");
			System.out.println("id est : " + id);

			SalarieService slrserv = new SalarieService();
			List<Formation> reponse = SalarieService.listerFormations();

			if (reponse.size() > 0) {
				request.setAttribute("listeFormation", reponse);
				RequestDispatcher rs = request.getRequestDispatcher("formations.jsp");
